...
def infer_user_intent(user_query, table_names_input):
    ...
    try:
        ...
        - OEEBREAKDOWNAI: Breakdown and downtime data (e.g., reasons, duration).
        - MACHINE_ACCESS_INFO_AI: Machine or plantcode  access information (e.g., machine ID,plantcode).
        Respond with a JSON object:
        {{
            "tables": ["table_name1", "table_name2" , "table_name3"] or ["table_name1"] or ["table_name2"] or ["table_name3"]
            "reason": "Explanation of table selection"
        }}
        If the query involves all tables (e.g., combining shift performance with breakdown reasons with specific plant access), include all.
        """
        response = client.chat.completions.create(
            model=GPT_MODEL,
            messages=[
...
hardcoded_token=token
# Attempt to decode the token at app startup
def initialize_user_session():
    if 'user_code' not in st.session_state or 'plant_code' not in st.session_state:
        user_code, plant_code, decoded_token = decode_jwt_token(hardcoded_token)
